...
import PostTypeFeaturedIcon from "@components/Atoms/PostTypeFeaturedIcon/PostTypeFeaturedIcon";
...
const PostFeaturedMedia: FC<PostFeaturedMediaProps> = ({
  ...
  const { featuredImage, postType, category, videoUrl, galleryImgs, audioUrl, id, href } =
    post;
  ...
  return (
    <div className={`nc-PostFeaturedMedia relative ${className}`}>
      ...
      {postType !== "gallery" && (
        <Link
        href={`/categorias/${category?.data?.attributes?.name.toLowerCase()}/${href}`}
